...
//interface mit den Variablen, aber noch ohne Array -> Schritt 1!!!!
interface Todo {
    text: string;
    isChecked: boolean;
}

//ein Array mit dem Interface -> Schritt 1!!!
var todos: Todo[] = [
    {
...
//Variablen für Counter für open und done wurden ergänzt -> Schritt 3!!!
...
window.addEventListener("load", function(): void {
    ...
    //Ergänzung für open und done -> Schritt 3!!!!
...
function updateCounter(): void {
    ...
    //brauchen eine Variable als Referenz, 
    let counterChecked: number = 0; //Zähler für erledigte Aufgaben
    let counterOpen: number = 0;  //Zähler für offene Aufgaben

    //eine Schleife, um nach und nach durchzugehen, um zu schauen, ob true or false ist
    for (let i: number = 0; i < todos.length; i++) { //i als zählende Variable
        console.log(todos[i].isChecked);
        if (todos[i].isChecked == true ) {
            //Counter soll um eins erhöht werden, wenn richtig (i++) (wenn todos an der Stelle 0 bei isChecked true ist)
            counterChecked++;
        } else {
            //Counter soll erhöht werden, wenn die if-Schleife nicht erfüllt wird
            counterOpen++;
        }
    }
...
function addTodo(): void {
    ...
    if (inputDOMElement.value != "") {
        ...
        //unshift statt push, damit Element an den Anfang hinzugefügt wird, statt am Ende -> Schritt 2!!!
...
function toggleCheckState(index: number): void {
     ...
     * Index für Zahl, damit man weiß, an welcher Stelle man ist
     */
